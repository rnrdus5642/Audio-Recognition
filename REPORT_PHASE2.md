...
> 보관용 실험 기록입니다. 아래 수치는 당시 모델·TTS 골든셋·매칭 설정의 결과이며,
> 현재 실시간 UI나 새 한글 발음 설명의 품질 평가가 아닙니다. 최신 사용법·테스트 범위는
> [README](README.md), 개발 변경은 [변경 기록](CHANGELOG.md)을 참고하세요.
...
> **⚠ 3차 업데이트 (경쟁 단어 제거)**: 이 보고서의 positives는 "정답이 같은
> 세그먼트 안에서 1등이고 임계값도 넘었을 때"로 셌다. 그 경쟁을 없애고
> **물어본 단어만 채점**하도록 바꾼 뒤 다시 재보니 positives는 25/36 = 69.4%로
> **완전히 동일**했다. 걸러내는 일은 단어별 임계값과 `skip_cost`가 하고 있었고,
> 경쟁은 앱이 "지금 화면에 어떤 단어들이 있는지"를 알아야 하게 만들 뿐이었다.
>
> negatives는 정의가 바뀌어 직접 비교할 수 없다. 전에는 "다른 세그먼트 4개"
> (144 케이스), 지금은 "다른 단어 17개"(612 케이스)를 대조한다.
>
> | 지표 | 세그먼트 경쟁 | 단어별 (현재) |
> |---|---|---|
> | Positives | 25/36 = 69.4% | 25/36 = 69.4% |
> | Negatives 거절 | 132/144 = 91.7% | 586/612 = 95.8% |
>
> 같은 실험에서 **연속 확인 기본값도 3 → 2로 내렸다**. 골든 클립 36개로 만든
> 스트리밍 세션에서 1회와 2회의 검출이 18/36으로 같고(2회가 오발동만 줄임),
> 3회는 15/36으로 떨어진다. 자세한 표는 `streaming.py` 주석에 있다.
