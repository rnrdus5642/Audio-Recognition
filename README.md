...
| 정답 음소 수 | 1–2 | 3–4 | 5 | 6 | 7 | 8+ |
|---|---|---|---|---|---|---|
| 임계값 | 0.85 | 0.825 | 0.80 | 0.775 | 0.825 | 0.875 |

**이 값들은 `skip_cost` 0.02 와 한 몸입니다.** 따로 읽으면 안 됩니다.

스트리밍은 예산 대부분을 건너뛰기에 씁니다 — 정답은 2.5초 중 일부만 차지하고
나머지는 지나쳐야 하니까요. 그래서 건너뛰기가 싸지면 맞는 것도 틀린 것도
점수가 같이 올라가고, 선도 따라 올라가야 합니다.

말뭉치 100단어를 Zeroth 성인 발화에 걸어 잰 결과입니다.

| | 검출 | 오확정 (10초·단어당) |
|---|---|---|
| 이전 (`skip_cost` 0.05 · 낮은 임계값) | 84.1% | 2.56% |
| `skip_cost` 만 0.02 로 | 91.6% | **16.59%** |
| 임계값만 올림 | **43.0%** | 0.68% |
| **둘 다** | **88.6%** | **1.35%** |

하나만 바꾸면 바꾼 방향으로 무너집니다. 커리큘럼 18단어 기준으로는 검출
90% → 100%, 오확정 0.48% → 0.29% 였습니다.

긴 단어가 짧은 단어보다 엄격한 것은 처음 가정과 반대입니다. 음소가 많으면
긴 문장 안에 통과할 구간이 생기기 쉽습니다.
...
떨어지고, 스트리밍 설정을 배치에 쓰면 positives 가 무너집니다 (커버리지 0.9
가 "아빠 → 앞" 같은 의도된 관용을 죽입니다).
