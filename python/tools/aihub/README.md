...
### 화자 ID 는 `NumberOfSpeaker` 입니다

`SpeakerName` 을 쓰면 안 됩니다. 이니셜이라 서로 다른 아이가 같은 값을
가집니다. 검증 아카이브 하나만 봐도 **아이 110명이 이름 99개를 나눠 쓰고**
있었고, 그중 5개는 나이까지 달랐습니다.

이걸로 분할하면 다른 아이들이 한 명으로 합쳐집니다. 실제로 5~7세 화자가
738명으로 보였는데, 제대로 세니 **1,400명**이었습니다.

`Basic/NumberOfSpeaker` 는 아이별 폴더명과 정확히 일치하고, free 와
formatted 아카이브에 걸쳐서도 겹치지 않으며, 한 값에 나이·성별이 하나씩만
붙습니다.

이 값을 `sha256(...)[:12]` 로 해시해서 씁니다. 분할에 필요한 건 "같은 아이가
양쪽에 걸치지 않는 것"뿐이라 원본 값을 들고 있을 이유가 없습니다.

`verify.py` 가 "화자당 나이 하나"를 검사하는 게 이 실수를 잡는 장치입니다.
