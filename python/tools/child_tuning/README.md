...
### 어휘는 늘리지 않습니다 (2026-08-18 결정)

어휘는 음절 1205개입니다. AI Hub 아동 전사를 대보면 **거기 없는 음절이
0.75%** 있습니다 (440종). 발화 단위로는 11.2% 가 하나 이상 걸립니다.

늘리면 출력층이 1205 → 약 1650 이 되고, 모델별 `vocab.json` 분리 · 패리티
벡터 재생성 · Unity 디코더 재확인이 따라옵니다. 어휘를 그대로 두면
디코더도 ONNX 출력 모양도 전부 유지됩니다.

**그대로 둡니다.** 어휘 밖 음절이 든 발화 11% 를 버려도 600시간이 남고,
어차피 학습에는 150~200시간만 씁니다.

대가는 하나뿐입니다.

```
놔    어휘에 없음 → 어떤 발음을 해도 전사 불가
      커리큘럼 `놔`·`놔요` 가 여기 걸림
      코퍼스에도 64건뿐이라 어휘를 늘려도 제대로 안 배움
```

29단어 중 이 둘만 해당되고, 이건 어휘 문제가 아니라 **단어 선정 문제**입니다.
다른 동사로 바꾸는 것이 맞습니다.
...
이 계산은 화자를 이니셜로 구분한 상태에서 한 것이라, 서로 다른 아이가 한
명으로 섞여 있었습니다 (`SpeakerName` 문제 — [tools/aihub](../aihub/README.md)
참고). 섞이면 아이별 차이가 평균으로 상쇄되므로 실제 개인차는 29%보다
**크면 컸지 작지 않습니다.** 결론은 그대로입니다.
