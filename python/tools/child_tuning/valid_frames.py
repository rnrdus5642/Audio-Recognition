"""Frames for the validation split: free speech, ages 8-9.

Same recording conditions as the tuning set and different children, so
the gap between the two measures speaker overfitting and nothing else.
The 5-year-old formatted set changes two things at once and is kept for
the final check.
"""
...
OUT = os.path.join(BASE, "valid_frames.json")
...
rows = read("08세/free") + read("09세/free")
...
print(f"검출 {len(pos)} + 오발동 {len(neg)} · {secs/3600:.1f}시간 · "
      f"화자 {len({r['speaker'] for r in pos + neg})}명", flush=True)
...
for name, rows_ in (("valid_pos", pos), ("valid_neg", neg)):
    items = []
    for r in rows_:
